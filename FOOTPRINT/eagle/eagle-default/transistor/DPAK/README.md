



Contents
========

* [FZETRANSISTORDPAK > transistor : DPAK](#fzetransistordpak--transistor--dpak)
	* [Tags](#tags)
  
NO IMAGE  
# FZETRANSISTORDPAK > transistor : DPAK

- ID: FOOTPRINT-eagle-eagle-default-transistor-DPAK
- Hex ID: FZETRANSISTORDPAK
- Name: transistor : DPAK
- Description: transistor : DPAK
- Long Link: [http://oom.lt/FOOTPRINT-eagle-eagle-default-transistor-DPAK](http://oom.lt/FOOTPRINT-eagle-eagle-default-transistor-DPAK)
- Short Link: [http://oom.lt/FZETRANSISTORDPAK](http://oom.lt/FZETRANSISTORDPAK)
  
NO IMAGES  
## Tags
  

|Tag Name|Tag Code|Tag Value|
| :--- | :--- | :--- |
|OOMP Type|oompType|FOOTPRINT|
|OOMP Size|oompSize|eagle|
|OOMP Color|oompColor|eagle-default|
|OOMP Description|oompDesc|transistor|
|OOMP Index|oompIndex|DPAK|
|Full OOMP ID|oompID|FOOTPRINT-eagle-eagle-default-transistor-DPAK|
|Hex ID (short code)|hexID|FZETRANSISTORDPAK|
|name|name|transistor : DPAK|
||||
