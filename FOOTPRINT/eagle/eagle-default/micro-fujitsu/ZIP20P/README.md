



Contents
========

* [FZEMFUJITSUZIP2P > micro-fujitsu : ZIP20P](#fzemfujitsuzip2p--micro-fujitsu--zip20p)
	* [Tags](#tags)
  
NO IMAGE  
# FZEMFUJITSUZIP2P > micro-fujitsu : ZIP20P

- ID: FOOTPRINT-eagle-eagle-default-micro-fujitsu-ZIP20P
- Hex ID: FZEMFUJITSUZIP2P
- Name: micro-fujitsu : ZIP20P
- Description: micro-fujitsu : ZIP20P
- Long Link: [http://oom.lt/FOOTPRINT-eagle-eagle-default-micro-fujitsu-ZIP20P](http://oom.lt/FOOTPRINT-eagle-eagle-default-micro-fujitsu-ZIP20P)
- Short Link: [http://oom.lt/FZEMFUJITSUZIP2P](http://oom.lt/FZEMFUJITSUZIP2P)
  
NO IMAGES  
## Tags
  

|Tag Name|Tag Code|Tag Value|
| :--- | :--- | :--- |
|OOMP Type|oompType|FOOTPRINT|
|OOMP Size|oompSize|eagle|
|OOMP Color|oompColor|eagle-default|
|OOMP Description|oompDesc|micro-fujitsu|
|OOMP Index|oompIndex|ZIP20P|
|Full OOMP ID|oompID|FOOTPRINT-eagle-eagle-default-micro-fujitsu-ZIP20P|
|Hex ID (short code)|hexID|FZEMFUJITSUZIP2P|
|name|name|micro-fujitsu : ZIP20P|
||||
