



Contents
========

* [FZEMCHIPDFNS > microchip : DFN-S](#fzemchipdfns--microchip--dfn-s)
	* [Tags](#tags)
  
NO IMAGE  
# FZEMCHIPDFNS > microchip : DFN-S

- ID: FOOTPRINT-eagle-eagle-default-microchip-DFN-S
- Hex ID: FZEMCHIPDFNS
- Name: microchip : DFN-S
- Description: microchip : DFN-S
- Long Link: [http://oom.lt/FOOTPRINT-eagle-eagle-default-microchip-DFN-S](http://oom.lt/FOOTPRINT-eagle-eagle-default-microchip-DFN-S)
- Short Link: [http://oom.lt/FZEMCHIPDFNS](http://oom.lt/FZEMCHIPDFNS)
  
NO IMAGES  
## Tags
  

|Tag Name|Tag Code|Tag Value|
| :--- | :--- | :--- |
|OOMP Type|oompType|FOOTPRINT|
|OOMP Size|oompSize|eagle|
|OOMP Color|oompColor|eagle-default|
|OOMP Description|oompDesc|microchip|
|OOMP Index|oompIndex|DFN-S|
|Full OOMP ID|oompID|FOOTPRINT-eagle-eagle-default-microchip-DFN-S|
|Hex ID (short code)|hexID|FZEMCHIPDFNS|
|name|name|microchip : DFN-S|
||||
