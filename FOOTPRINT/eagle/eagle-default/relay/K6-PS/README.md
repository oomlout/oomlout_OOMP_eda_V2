



Contents
========

* [FZERELAYK6PS > relay : K6-PS](#fzerelayk6ps--relay--k6-ps)
	* [Tags](#tags)
  
NO IMAGE  
# FZERELAYK6PS > relay : K6-PS

- ID: FOOTPRINT-eagle-eagle-default-relay-K6-PS
- Hex ID: FZERELAYK6PS
- Name: relay : K6-PS
- Description: relay : K6-PS
- Long Link: [http://oom.lt/FOOTPRINT-eagle-eagle-default-relay-K6-PS](http://oom.lt/FOOTPRINT-eagle-eagle-default-relay-K6-PS)
- Short Link: [http://oom.lt/FZERELAYK6PS](http://oom.lt/FZERELAYK6PS)
  
NO IMAGES  
## Tags
  

|Tag Name|Tag Code|Tag Value|
| :--- | :--- | :--- |
|OOMP Type|oompType|FOOTPRINT|
|OOMP Size|oompSize|eagle|
|OOMP Color|oompColor|eagle-default|
|OOMP Description|oompDesc|relay|
|OOMP Index|oompIndex|K6-PS|
|Full OOMP ID|oompID|FOOTPRINT-eagle-eagle-default-relay-K6-PS|
|Hex ID (short code)|hexID|FZERELAYK6PS|
|name|name|relay : K6-PS|
||||
