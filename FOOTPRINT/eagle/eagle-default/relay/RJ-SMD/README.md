



Contents
========

* [FZERELAYRJSM > relay : RJ-SMD](#fzerelayrjsm--relay--rj-smd)
	* [Tags](#tags)
  
NO IMAGE  
# FZERELAYRJSM > relay : RJ-SMD

- ID: FOOTPRINT-eagle-eagle-default-relay-RJ-SMD
- Hex ID: FZERELAYRJSM
- Name: relay : RJ-SMD
- Description: relay : RJ-SMD
- Long Link: [http://oom.lt/FOOTPRINT-eagle-eagle-default-relay-RJ-SMD](http://oom.lt/FOOTPRINT-eagle-eagle-default-relay-RJ-SMD)
- Short Link: [http://oom.lt/FZERELAYRJSM](http://oom.lt/FZERELAYRJSM)
  
NO IMAGES  
## Tags
  

|Tag Name|Tag Code|Tag Value|
| :--- | :--- | :--- |
|OOMP Type|oompType|FOOTPRINT|
|OOMP Size|oompSize|eagle|
|OOMP Color|oompColor|eagle-default|
|OOMP Description|oompDesc|relay|
|OOMP Index|oompIndex|RJ-SMD|
|Full OOMP ID|oompID|FOOTPRINT-eagle-eagle-default-relay-RJ-SMD|
|Hex ID (short code)|hexID|FZERELAYRJSM|
|name|name|relay : RJ-SMD|
||||
