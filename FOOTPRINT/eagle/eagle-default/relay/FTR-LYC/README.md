



Contents
========

* [FZERELAYFTRLYC > relay : FTR-LYC](#fzerelayftrlyc--relay--ftr-lyc)
	* [Tags](#tags)
  
NO IMAGE  
# FZERELAYFTRLYC > relay : FTR-LYC

- ID: FOOTPRINT-eagle-eagle-default-relay-FTR-LYC
- Hex ID: FZERELAYFTRLYC
- Name: relay : FTR-LYC
- Description: relay : FTR-LYC
- Long Link: [http://oom.lt/FOOTPRINT-eagle-eagle-default-relay-FTR-LYC](http://oom.lt/FOOTPRINT-eagle-eagle-default-relay-FTR-LYC)
- Short Link: [http://oom.lt/FZERELAYFTRLYC](http://oom.lt/FZERELAYFTRLYC)
  
NO IMAGES  
## Tags
  

|Tag Name|Tag Code|Tag Value|
| :--- | :--- | :--- |
|OOMP Type|oompType|FOOTPRINT|
|OOMP Size|oompSize|eagle|
|OOMP Color|oompColor|eagle-default|
|OOMP Description|oompDesc|relay|
|OOMP Index|oompIndex|FTR-LYC|
|Full OOMP ID|oompID|FOOTPRINT-eagle-eagle-default-relay-FTR-LYC|
|Hex ID (short code)|hexID|FZERELAYFTRLYC|
|name|name|relay : FTR-LYC|
||||
