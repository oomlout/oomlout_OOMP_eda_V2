



Contents
========

* [FZERELAYJW1A > relay : JW1A](#fzerelayjw1a--relay--jw1a)
	* [Tags](#tags)
  
NO IMAGE  
# FZERELAYJW1A > relay : JW1A

- ID: FOOTPRINT-eagle-eagle-default-relay-JW1A
- Hex ID: FZERELAYJW1A
- Name: relay : JW1A
- Description: relay : JW1A
- Long Link: [http://oom.lt/FOOTPRINT-eagle-eagle-default-relay-JW1A](http://oom.lt/FOOTPRINT-eagle-eagle-default-relay-JW1A)
- Short Link: [http://oom.lt/FZERELAYJW1A](http://oom.lt/FZERELAYJW1A)
  
NO IMAGES  
## Tags
  

|Tag Name|Tag Code|Tag Value|
| :--- | :--- | :--- |
|OOMP Type|oompType|FOOTPRINT|
|OOMP Size|oompSize|eagle|
|OOMP Color|oompColor|eagle-default|
|OOMP Description|oompDesc|relay|
|OOMP Index|oompIndex|JW1A|
|Full OOMP ID|oompID|FOOTPRINT-eagle-eagle-default-relay-JW1A|
|Hex ID (short code)|hexID|FZERELAYJW1A|
|name|name|relay : JW1A|
||||
