



Contents
========

* [FZERELAYAQB > relay : AQ-B](#fzerelayaqb--relay--aq-b)
	* [Tags](#tags)
  
NO IMAGE  
# FZERELAYAQB > relay : AQ-B

- ID: FOOTPRINT-eagle-eagle-default-relay-AQ-B
- Hex ID: FZERELAYAQB
- Name: relay : AQ-B
- Description: relay : AQ-B
- Long Link: [http://oom.lt/FOOTPRINT-eagle-eagle-default-relay-AQ-B](http://oom.lt/FOOTPRINT-eagle-eagle-default-relay-AQ-B)
- Short Link: [http://oom.lt/FZERELAYAQB](http://oom.lt/FZERELAYAQB)
  
NO IMAGES  
## Tags
  

|Tag Name|Tag Code|Tag Value|
| :--- | :--- | :--- |
|OOMP Type|oompType|FOOTPRINT|
|OOMP Size|oompSize|eagle|
|OOMP Color|oompColor|eagle-default|
|OOMP Description|oompDesc|relay|
|OOMP Index|oompIndex|AQ-B|
|Full OOMP ID|oompID|FOOTPRINT-eagle-eagle-default-relay-AQ-B|
|Hex ID (short code)|hexID|FZERELAYAQB|
|name|name|relay : AQ-B|
||||
