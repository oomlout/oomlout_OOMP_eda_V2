



Contents
========

* [FZERELAYRKL2 > relay : RKL2](#fzerelayrkl2--relay--rkl2)
	* [Tags](#tags)
  
NO IMAGE  
# FZERELAYRKL2 > relay : RKL2

- ID: FOOTPRINT-eagle-eagle-default-relay-RKL2
- Hex ID: FZERELAYRKL2
- Name: relay : RKL2
- Description: relay : RKL2
- Long Link: [http://oom.lt/FOOTPRINT-eagle-eagle-default-relay-RKL2](http://oom.lt/FOOTPRINT-eagle-eagle-default-relay-RKL2)
- Short Link: [http://oom.lt/FZERELAYRKL2](http://oom.lt/FZERELAYRKL2)
  
NO IMAGES  
## Tags
  

|Tag Name|Tag Code|Tag Value|
| :--- | :--- | :--- |
|OOMP Type|oompType|FOOTPRINT|
|OOMP Size|oompSize|eagle|
|OOMP Color|oompColor|eagle-default|
|OOMP Description|oompDesc|relay|
|OOMP Index|oompIndex|RKL2|
|Full OOMP ID|oompID|FOOTPRINT-eagle-eagle-default-relay-RKL2|
|Hex ID (short code)|hexID|FZERELAYRKL2|
|name|name|relay : RKL2|
||||
