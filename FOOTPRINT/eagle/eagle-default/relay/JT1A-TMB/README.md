



Contents
========

* [FZERELAYJT1ATMB > relay : JT1A-TMB](#fzerelayjt1atmb--relay--jt1a-tmb)
	* [Tags](#tags)
  
NO IMAGE  
# FZERELAYJT1ATMB > relay : JT1A-TMB

- ID: FOOTPRINT-eagle-eagle-default-relay-JT1A-TMB
- Hex ID: FZERELAYJT1ATMB
- Name: relay : JT1A-TMB
- Description: relay : JT1A-TMB
- Long Link: [http://oom.lt/FOOTPRINT-eagle-eagle-default-relay-JT1A-TMB](http://oom.lt/FOOTPRINT-eagle-eagle-default-relay-JT1A-TMB)
- Short Link: [http://oom.lt/FZERELAYJT1ATMB](http://oom.lt/FZERELAYJT1ATMB)
  
NO IMAGES  
## Tags
  

|Tag Name|Tag Code|Tag Value|
| :--- | :--- | :--- |
|OOMP Type|oompType|FOOTPRINT|
|OOMP Size|oompSize|eagle|
|OOMP Color|oompColor|eagle-default|
|OOMP Description|oompDesc|relay|
|OOMP Index|oompIndex|JT1A-TMB|
|Full OOMP ID|oompID|FOOTPRINT-eagle-eagle-default-relay-JT1A-TMB|
|Hex ID (short code)|hexID|FZERELAYJT1ATMB|
|name|name|relay : JT1A-TMB|
||||
