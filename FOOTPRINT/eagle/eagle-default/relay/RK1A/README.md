



Contents
========

* [FZERELAYRK1A > relay : RK1A](#fzerelayrk1a--relay--rk1a)
	* [Tags](#tags)
  
NO IMAGE  
# FZERELAYRK1A > relay : RK1A

- ID: FOOTPRINT-eagle-eagle-default-relay-RK1A
- Hex ID: FZERELAYRK1A
- Name: relay : RK1A
- Description: relay : RK1A
- Long Link: [http://oom.lt/FOOTPRINT-eagle-eagle-default-relay-RK1A](http://oom.lt/FOOTPRINT-eagle-eagle-default-relay-RK1A)
- Short Link: [http://oom.lt/FZERELAYRK1A](http://oom.lt/FZERELAYRK1A)
  
NO IMAGES  
## Tags
  

|Tag Name|Tag Code|Tag Value|
| :--- | :--- | :--- |
|OOMP Type|oompType|FOOTPRINT|
|OOMP Size|oompSize|eagle|
|OOMP Color|oompColor|eagle-default|
|OOMP Description|oompDesc|relay|
|OOMP Index|oompIndex|RK1A|
|Full OOMP ID|oompID|FOOTPRINT-eagle-eagle-default-relay-RK1A|
|Hex ID (short code)|hexID|FZERELAYRK1A|
|name|name|relay : RK1A|
||||
