



Contents
========

* [FZERELAYTQSS > relay : TQ-SS](#fzerelaytqss--relay--tq-ss)
	* [Tags](#tags)
  
NO IMAGE  
# FZERELAYTQSS > relay : TQ-SS

- ID: FOOTPRINT-eagle-eagle-default-relay-TQ-SS
- Hex ID: FZERELAYTQSS
- Name: relay : TQ-SS
- Description: relay : TQ-SS
- Long Link: [http://oom.lt/FOOTPRINT-eagle-eagle-default-relay-TQ-SS](http://oom.lt/FOOTPRINT-eagle-eagle-default-relay-TQ-SS)
- Short Link: [http://oom.lt/FZERELAYTQSS](http://oom.lt/FZERELAYTQSS)
  
NO IMAGES  
## Tags
  

|Tag Name|Tag Code|Tag Value|
| :--- | :--- | :--- |
|OOMP Type|oompType|FOOTPRINT|
|OOMP Size|oompSize|eagle|
|OOMP Color|oompColor|eagle-default|
|OOMP Description|oompDesc|relay|
|OOMP Index|oompIndex|TQ-SS|
|Full OOMP ID|oompID|FOOTPRINT-eagle-eagle-default-relay-TQ-SS|
|Hex ID (short code)|hexID|FZERELAYTQSS|
|name|name|relay : TQ-SS|
||||
