



Contents
========

* [FZETEXASCBB > texas : CBB](#fzetexascbb--texas--cbb)
	* [Tags](#tags)
  
NO IMAGE  
# FZETEXASCBB > texas : CBB

- ID: FOOTPRINT-eagle-eagle-default-texas-CBB
- Hex ID: FZETEXASCBB
- Name: texas : CBB
- Description: texas : CBB
- Long Link: [http://oom.lt/FOOTPRINT-eagle-eagle-default-texas-CBB](http://oom.lt/FOOTPRINT-eagle-eagle-default-texas-CBB)
- Short Link: [http://oom.lt/FZETEXASCBB](http://oom.lt/FZETEXASCBB)
  
NO IMAGES  
## Tags
  

|Tag Name|Tag Code|Tag Value|
| :--- | :--- | :--- |
|OOMP Type|oompType|FOOTPRINT|
|OOMP Size|oompSize|eagle|
|OOMP Color|oompColor|eagle-default|
|OOMP Description|oompDesc|texas|
|OOMP Index|oompIndex|CBB|
|Full OOMP ID|oompID|FOOTPRINT-eagle-eagle-default-texas-CBB|
|Hex ID (short code)|hexID|FZETEXASCBB|
|name|name|texas : CBB|
||||
