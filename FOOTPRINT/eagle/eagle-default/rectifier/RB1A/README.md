



Contents
========

* [FZERECTIFIERRB1A > rectifier : RB1A](#fzerectifierrb1a--rectifier--rb1a)
	* [Tags](#tags)
  
NO IMAGE  
# FZERECTIFIERRB1A > rectifier : RB1A

- ID: FOOTPRINT-eagle-eagle-default-rectifier-RB1A
- Hex ID: FZERECTIFIERRB1A
- Name: rectifier : RB1A
- Description: rectifier : RB1A
- Long Link: [http://oom.lt/FOOTPRINT-eagle-eagle-default-rectifier-RB1A](http://oom.lt/FOOTPRINT-eagle-eagle-default-rectifier-RB1A)
- Short Link: [http://oom.lt/FZERECTIFIERRB1A](http://oom.lt/FZERECTIFIERRB1A)
  
NO IMAGES  
## Tags
  

|Tag Name|Tag Code|Tag Value|
| :--- | :--- | :--- |
|OOMP Type|oompType|FOOTPRINT|
|OOMP Size|oompSize|eagle|
|OOMP Color|oompColor|eagle-default|
|OOMP Description|oompDesc|rectifier|
|OOMP Index|oompIndex|RB1A|
|Full OOMP ID|oompID|FOOTPRINT-eagle-eagle-default-rectifier-RB1A|
|Hex ID (short code)|hexID|FZERECTIFIERRB1A|
|name|name|rectifier : RB1A|
||||
