



Contents
========

* [FZSRFIOTA > SparkFun-RF : IOTA](#fzsrfiota--sparkfun-rf--iota)
	* [Tags](#tags)
  
NO IMAGE  
# FZSRFIOTA > SparkFun-RF : IOTA

- ID: FOOTPRINT-eagle-SparkFun-Eagle-Libraries-SparkFun-RF-IOTA
- Hex ID: FZSRFIOTA
- Name: SparkFun-RF : IOTA
- Description: SparkFun-RF : IOTA
- Long Link: [http://oom.lt/FOOTPRINT-eagle-SparkFun-Eagle-Libraries-SparkFun-RF-IOTA](http://oom.lt/FOOTPRINT-eagle-SparkFun-Eagle-Libraries-SparkFun-RF-IOTA)
- Short Link: [http://oom.lt/FZSRFIOTA](http://oom.lt/FZSRFIOTA)
  
NO IMAGES  
## Tags
  

|Tag Name|Tag Code|Tag Value|
| :--- | :--- | :--- |
|OOMP Type|oompType|FOOTPRINT|
|OOMP Size|oompSize|eagle|
|OOMP Color|oompColor|SparkFun-Eagle-Libraries|
|OOMP Description|oompDesc|SparkFun-RF|
|OOMP Index|oompIndex|IOTA|
|Full OOMP ID|oompID|FOOTPRINT-eagle-SparkFun-Eagle-Libraries-SparkFun-RF-IOTA|
|Hex ID (short code)|hexID|FZSRFIOTA|
|name|name|SparkFun-RF : IOTA|
||||
