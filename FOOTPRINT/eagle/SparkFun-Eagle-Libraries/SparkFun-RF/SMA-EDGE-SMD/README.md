



Contents
========

* [FZSRFSEDGESM > SparkFun-RF : SMA-EDGE-SMD](#fzsrfsedgesm--sparkfun-rf--sma-edge-smd)
	* [Tags](#tags)
  
NO IMAGE  
# FZSRFSEDGESM > SparkFun-RF : SMA-EDGE-SMD

- ID: FOOTPRINT-eagle-SparkFun-Eagle-Libraries-SparkFun-RF-SMA-EDGE-SMD
- Hex ID: FZSRFSEDGESM
- Name: SparkFun-RF : SMA-EDGE-SMD
- Description: SparkFun-RF : SMA-EDGE-SMD
- Long Link: [http://oom.lt/FOOTPRINT-eagle-SparkFun-Eagle-Libraries-SparkFun-RF-SMA-EDGE-SMD](http://oom.lt/FOOTPRINT-eagle-SparkFun-Eagle-Libraries-SparkFun-RF-SMA-EDGE-SMD)
- Short Link: [http://oom.lt/FZSRFSEDGESM](http://oom.lt/FZSRFSEDGESM)
  
NO IMAGES  
## Tags
  

|Tag Name|Tag Code|Tag Value|
| :--- | :--- | :--- |
|OOMP Type|oompType|FOOTPRINT|
|OOMP Size|oompSize|eagle|
|OOMP Color|oompColor|SparkFun-Eagle-Libraries|
|OOMP Description|oompDesc|SparkFun-RF|
|OOMP Index|oompIndex|SMA-EDGE-SMD|
|Full OOMP ID|oompID|FOOTPRINT-eagle-SparkFun-Eagle-Libraries-SparkFun-RF-SMA-EDGE-SMD|
|Hex ID (short code)|hexID|FZSRFSEDGESM|
|name|name|SparkFun-RF : SMA-EDGE-SMD|
||||
