



Contents
========

* [FZSRTPOTP > SparkFun-Retired : POT_PTH](#fzsrtpotp--sparkfun-retired--pot_pth)
	* [Tags](#tags)
  
NO IMAGE  
# FZSRTPOTP > SparkFun-Retired : POT_PTH

- ID: FOOTPRINT-eagle-SparkFun-Eagle-Libraries-SparkFun-Retired-POT_PTH
- Hex ID: FZSRTPOTP
- Name: SparkFun-Retired : POT_PTH
- Description: SparkFun-Retired : POT_PTH
- Long Link: [http://oom.lt/FOOTPRINT-eagle-SparkFun-Eagle-Libraries-SparkFun-Retired-POT_PTH](http://oom.lt/FOOTPRINT-eagle-SparkFun-Eagle-Libraries-SparkFun-Retired-POT_PTH)
- Short Link: [http://oom.lt/FZSRTPOTP](http://oom.lt/FZSRTPOTP)
  
NO IMAGES  
## Tags
  

|Tag Name|Tag Code|Tag Value|
| :--- | :--- | :--- |
|OOMP Type|oompType|FOOTPRINT|
|OOMP Size|oompSize|eagle|
|OOMP Color|oompColor|SparkFun-Eagle-Libraries|
|OOMP Description|oompDesc|SparkFun-Retired|
|OOMP Index|oompIndex|POT_PTH|
|Full OOMP ID|oompID|FOOTPRINT-eagle-SparkFun-Eagle-Libraries-SparkFun-Retired-POT_PTH|
|Hex ID (short code)|hexID|FZSRTPOTP|
|name|name|SparkFun-Retired : POT_PTH|
||||
