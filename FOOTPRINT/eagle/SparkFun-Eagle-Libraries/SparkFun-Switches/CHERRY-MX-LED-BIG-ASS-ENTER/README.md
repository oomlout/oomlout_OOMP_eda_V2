



Contents
========

* [FZSWCHERRYMXLBASSENTER > SparkFun-Switches : CHERRY-MX-LED-BIG-ASS-ENTER](#fzswcherrymxlbassenter--sparkfun-switches--cherry-mx-led-big-ass-enter)
	* [Tags](#tags)
  
NO IMAGE  
# FZSWCHERRYMXLBASSENTER > SparkFun-Switches : CHERRY-MX-LED-BIG-ASS-ENTER

- ID: FOOTPRINT-eagle-SparkFun-Eagle-Libraries-SparkFun-Switches-CHERRY-MX-LED-BIG-ASS-ENTER
- Hex ID: FZSWCHERRYMXLBASSENTER
- Name: SparkFun-Switches : CHERRY-MX-LED-BIG-ASS-ENTER
- Description: SparkFun-Switches : CHERRY-MX-LED-BIG-ASS-ENTER
- Long Link: [http://oom.lt/FOOTPRINT-eagle-SparkFun-Eagle-Libraries-SparkFun-Switches-CHERRY-MX-LED-BIG-ASS-ENTER](http://oom.lt/FOOTPRINT-eagle-SparkFun-Eagle-Libraries-SparkFun-Switches-CHERRY-MX-LED-BIG-ASS-ENTER)
- Short Link: [http://oom.lt/FZSWCHERRYMXLBASSENTER](http://oom.lt/FZSWCHERRYMXLBASSENTER)
  
NO IMAGES  
## Tags
  

|Tag Name|Tag Code|Tag Value|
| :--- | :--- | :--- |
|OOMP Type|oompType|FOOTPRINT|
|OOMP Size|oompSize|eagle|
|OOMP Color|oompColor|SparkFun-Eagle-Libraries|
|OOMP Description|oompDesc|SparkFun-Switches|
|OOMP Index|oompIndex|CHERRY-MX-LED-BIG-ASS-ENTER|
|Full OOMP ID|oompID|FOOTPRINT-eagle-SparkFun-Eagle-Libraries-SparkFun-Switches-CHERRY-MX-LED-BIG-ASS-ENTER|
|Hex ID (short code)|hexID|FZSWCHERRYMXLBASSENTER|
|name|name|SparkFun-Switches : CHERRY-MX-LED-BIG-ASS-ENTER|
||||
