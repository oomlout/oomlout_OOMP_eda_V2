



Contents
========

* [FZKSOTSOT233 > Package_TO_SOT_SMD : SOT-23-3](#fzksotsot233--package_to_sot_smd--sot-23-3)
	* [Tags](#tags)
  
NO IMAGE  
# FZKSOTSOT233 > Package_TO_SOT_SMD : SOT-23-3

- ID: FOOTPRINT-kicad-kicad-footprints-Package_TO_SOT_SMD-SOT-23-3
- Hex ID: FZKSOTSOT233
- Name: Package_TO_SOT_SMD : SOT-23-3
- Description: Package_TO_SOT_SMD : SOT-23-3
- Long Link: [http://oom.lt/FOOTPRINT-kicad-kicad-footprints-Package_TO_SOT_SMD-SOT-23-3](http://oom.lt/FOOTPRINT-kicad-kicad-footprints-Package_TO_SOT_SMD-SOT-23-3)
- Short Link: [http://oom.lt/FZKSOTSOT233](http://oom.lt/FZKSOTSOT233)
  
NO IMAGES  
## Tags
  

|Tag Name|Tag Code|Tag Value|
| :--- | :--- | :--- |
|OOMP Type|oompType|FOOTPRINT|
|OOMP Size|oompSize|kicad|
|OOMP Color|oompColor|kicad-footprints|
|OOMP Description|oompDesc|Package_TO_SOT_SMD|
|OOMP Index|oompIndex|SOT-23-3|
|Full OOMP ID|oompID|FOOTPRINT-kicad-kicad-footprints-Package_TO_SOT_SMD-SOT-23-3|
|Hex ID (short code)|hexID|FZKSOTSOT233|
|name|name|Package_TO_SOT_SMD : SOT-23-3|
|Footprint Kicad|footprintKicadDetails|<table><tr><td>'name'</td></tr><tr><td> 'SOT-23-3'</td><td> 'description'</td></tr><tr><td> 'SOT</td><td> 3 Pin (https</td></tr><tr><td>//www.jedec.org/sites/default/files/docs/Mo-178D.PDF inferred 3-pin variant)</td><td> generated with kicad-footprint-generator ipc_gullwing_generator.py'</td><td> 'tags'</td></tr><tr><td> 'SOT TO_SOT_SMD'</td><td> 'attributeType'</td></tr><tr><td> 'smd'</td><td> 'threeDModel'</td></tr><tr><td> '$<table><tr><td>KICAD6_3DMODEL_DIR</td></tr></table>/Package_TO_SOT_SMD.3dshapes/SOT-23-3.wrl'</td><td> 'pins'</td></tr><tr><td> <table><tr><td>'type'</td></tr><tr><td> 'smd'</td><td> 'shape'</td></tr><tr><td> 'roundrect'</td></tr></table></td></tr></table>|
||||
