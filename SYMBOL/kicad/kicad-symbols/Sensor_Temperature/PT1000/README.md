


  
NO IMAGE  
# SZKSENTEMPERATUREPT1 > PT1000

- ID: SYMBOL-kicad-kicad-symbols-Sensor_Temperature-PT1000
- Hex ID: SZKSENTEMPERATUREPT1
- Name: PT1000
- Description: PT1000
- Long Link: [http://oom.lt/SYMBOL-kicad-kicad-symbols-Sensor_Temperature-PT1000](http://oom.lt/SYMBOL-kicad-kicad-symbols-Sensor_Temperature-PT1000)
- Short Link: [http://oom.lt/SZKSENTEMPERATUREPT1](http://oom.lt/SZKSENTEMPERATUREPT1)
  

|oomlout_OOMP_eda_V2/SYMBOL/kicad/kicad-symbols/Sensor_Temperature/PT1000/image.png||||
| :---: | :---: | :---: | :---: |

## Tags
  

|Tag Name|Tag Code|Tag Value|
| :--- | :--- | :--- |
|OOMP Type|oompType|SYMBOL|
|OOMP Size|oompSize|kicad|
|OOMP Color|oompColor|kicad-symbols|
|OOMP Description|oompDesc|Sensor_Temperature|
|OOMP Index|oompIndex|PT1000|
|Full OOMP ID|oompID|SYMBOL-kicad-kicad-symbols-Sensor_Temperature-PT1000|
|Hex ID (short code)|hexID|SZKSENTEMPERATUREPT1|
|Name|name|PT1000|
|Symbol Kicad Details|symbolKicadDetails|[{'kicadSymbolExtends': 'PT100', 'kicadSymbolReference': 'TH', 'kicadSymbolValue': 'PT1000', 'kicadSymbolFootprint': '', 'kicadSymbolDatasheet': 'https://www.heraeus.com/media/media/group/doc_group/products_1/hst/sot_to/de_15/to_92_d.pdf', 'kicadSymbolki_keywords': 'platinum temperature sensor RTD', 'kicadSymbolki_description': 'PT1000 platinum temperature sensor (RTD)', 'kicadSymbolki_fp_filters': 'TO?92* PIN?ARRAY* bornier* Terminal?Block* SOD70* R*1206* R*0805*'}]|
||||
