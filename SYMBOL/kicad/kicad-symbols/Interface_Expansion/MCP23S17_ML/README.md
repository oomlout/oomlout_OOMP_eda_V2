


  
NO IMAGE  
# SZKINTERFACEEXPANSIONMCP23S17ML > MCP23S17_ML

- ID: SYMBOL-kicad-kicad-symbols-Interface_Expansion-MCP23S17_ML
- Hex ID: SZKINTERFACEEXPANSIONMCP23S17ML
- Name: MCP23S17_ML
- Description: MCP23S17_ML
- Long Link: [http://oom.lt/SYMBOL-kicad-kicad-symbols-Interface_Expansion-MCP23S17_ML](http://oom.lt/SYMBOL-kicad-kicad-symbols-Interface_Expansion-MCP23S17_ML)
- Short Link: [http://oom.lt/SZKINTERFACEEXPANSIONMCP23S17ML](http://oom.lt/SZKINTERFACEEXPANSIONMCP23S17ML)
  

|oomlout_OOMP_eda_V2/SYMBOL/kicad/kicad-symbols/Interface_Expansion/MCP23S17_ML/image.png||||
| :---: | :---: | :---: | :---: |

## Tags
  

|Tag Name|Tag Code|Tag Value|
| :--- | :--- | :--- |
|OOMP Type|oompType|SYMBOL|
|OOMP Size|oompSize|kicad|
|OOMP Color|oompColor|kicad-symbols|
|OOMP Description|oompDesc|Interface_Expansion|
|OOMP Index|oompIndex|MCP23S17_ML|
|Full OOMP ID|oompID|SYMBOL-kicad-kicad-symbols-Interface_Expansion-MCP23S17_ML|
|Hex ID (short code)|hexID|SZKINTERFACEEXPANSIONMCP23S17ML|
|Name|name|MCP23S17_ML|
|Symbol Kicad Details|symbolKicadDetails|[{'kicadSymbolExtends': None, 'kicadSymbolReference': 'U', 'kicadSymbolValue': 'MCP23S17_ML', 'kicadSymbolFootprint': 'Package_DFN_QFN:QFN-28-1EP_6x6mm_P0.65mm_EP4.25x4.25mm', 'kicadSymbolDatasheet': 'http://ww1.microchip.com/downloads/en/DeviceDoc/20001952C.pdf', 'kicadSymbolki_keywords': 'SPI parallel port expander', 'kicadSymbolki_description': '16-bit I/O expander, SPI, interrupts, w pull-ups, QFN-28', 'kicadSymbolki_fp_filters': 'QFN*6x6mm*P0.65mm*'}]|
||||
