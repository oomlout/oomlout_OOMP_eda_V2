


  
NO IMAGE  
# SZKMCUSTSTM32F1STM32F11TBUX > STM32F101TBUx

- ID: SYMBOL-kicad-kicad-symbols-MCU_ST_STM32F1-STM32F101TBUx
- Hex ID: SZKMCUSTSTM32F1STM32F11TBUX
- Name: STM32F101TBUx
- Description: STM32F101TBUx
- Long Link: [http://oom.lt/SYMBOL-kicad-kicad-symbols-MCU_ST_STM32F1-STM32F101TBUx](http://oom.lt/SYMBOL-kicad-kicad-symbols-MCU_ST_STM32F1-STM32F101TBUx)
- Short Link: [http://oom.lt/SZKMCUSTSTM32F1STM32F11TBUX](http://oom.lt/SZKMCUSTSTM32F1STM32F11TBUX)
  

|oomlout_OOMP_eda_V2/SYMBOL/kicad/kicad-symbols/MCU_ST_STM32F1/STM32F101TBUx/image.png||||
| :---: | :---: | :---: | :---: |

## Tags
  

|Tag Name|Tag Code|Tag Value|
| :--- | :--- | :--- |
|OOMP Type|oompType|SYMBOL|
|OOMP Size|oompSize|kicad|
|OOMP Color|oompColor|kicad-symbols|
|OOMP Description|oompDesc|MCU_ST_STM32F1|
|OOMP Index|oompIndex|STM32F101TBUx|
|Full OOMP ID|oompID|SYMBOL-kicad-kicad-symbols-MCU_ST_STM32F1-STM32F101TBUx|
|Hex ID (short code)|hexID|SZKMCUSTSTM32F1STM32F11TBUX|
|Name|name|STM32F101TBUx|
|Symbol Kicad Details|symbolKicadDetails|[{'kicadSymbolExtends': 'STM32F101T8Ux', 'kicadSymbolReference': 'U', 'kicadSymbolValue': 'STM32F101TBUx', 'kicadSymbolFootprint': 'Package_DFN_QFN:QFN-36-1EP_6x6mm_P0.5mm_EP4.1x4.1mm', 'kicadSymbolDatasheet': 'http://www.st.com/st-web-ui/static/active/en/resource/technical/document/datasheet/CD00161561.pdf', 'kicadSymbolki_keywords': 'ARM Cortex-M3 STM32F1 STM32F101', 'kicadSymbolki_description': 'ARM Cortex-M3 MCU, 128KB flash, 16KB RAM, 36MHz, 2-3.6V, 26 GPIO, VFQFPN-36', 'kicadSymbolki_fp_filters': 'QFN*1EP*6x6mm*P0.5mm*'}]|
||||
