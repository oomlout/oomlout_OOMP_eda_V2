


  
NO IMAGE  
# SZKCNUCRECEPTACLE > USB_C_Receptacle

- ID: SYMBOL-kicad-kicad-symbols-Connector-USB_C_Receptacle
- Hex ID: SZKCNUCRECEPTACLE
- Name: USB_C_Receptacle
- Description: USB_C_Receptacle
- Long Link: [http://oom.lt/SYMBOL-kicad-kicad-symbols-Connector-USB_C_Receptacle](http://oom.lt/SYMBOL-kicad-kicad-symbols-Connector-USB_C_Receptacle)
- Short Link: [http://oom.lt/SZKCNUCRECEPTACLE](http://oom.lt/SZKCNUCRECEPTACLE)
  

|oomlout_OOMP_eda_V2/SYMBOL/kicad/kicad-symbols/Connector/USB_C_Receptacle/image.png||||
| :---: | :---: | :---: | :---: |

## Tags
  

|Tag Name|Tag Code|Tag Value|
| :--- | :--- | :--- |
|OOMP Type|oompType|SYMBOL|
|OOMP Size|oompSize|kicad|
|OOMP Color|oompColor|kicad-symbols|
|OOMP Description|oompDesc|Connector|
|OOMP Index|oompIndex|USB_C_Receptacle|
|Full OOMP ID|oompID|SYMBOL-kicad-kicad-symbols-Connector-USB_C_Receptacle|
|Hex ID (short code)|hexID|SZKCNUCRECEPTACLE|
|Name|name|USB_C_Receptacle|
|Symbol Kicad Details|symbolKicadDetails|[{'kicadSymbolExtends': None, 'kicadSymbolReference': 'J', 'kicadSymbolValue': 'USB_C_Receptacle', 'kicadSymbolFootprint': '', 'kicadSymbolDatasheet': 'https://www.usb.org/sites/default/files/documents/usb_type-c.zip', 'kicadSymbolki_keywords': 'usb universal serial bus type-C full-featured', 'kicadSymbolki_description': 'USB Full-Featured Type-C Receptacle connector', 'kicadSymbolki_fp_filters': 'USB*C*Receptacle*'}]|
||||
